...
- [ ] Phase 1: Specification (write docs/SPEC.md)
- [ ] Phase 2: Architecture & Plan (write docs/PLAN.md)
...
**Goal**: Produce `docs/SPEC.md` — the single source of truth.
...
**Goal**: Produce `docs/PLAN.md` — a phased implementation roadmap designed
...
**Feature**: [name]
**Spec**: docs/SPEC.md — [X requirements, Y acceptance criteria]
**Plan**: docs/PLAN.md — [N implementation units]
...
1. Derive a slug from the feature name:
   `feature-slug` (lowercase, hyphens, no spaces)
...
gh issue create \
  ...
  --label "spec,plan,ready-for-implementation"
```
...
- Tag the handoff in the progress checklist:
  ```
  - [x] Phase 3: Approved by user. Handed off via [GH issue #N / docs/<slug>/]
  ```
