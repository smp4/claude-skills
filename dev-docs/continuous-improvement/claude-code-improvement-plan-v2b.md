...
1. **Load and analyze aggregate telemetry + tool-stats.json.**
   ...
   - Identify skill cost/value concerns (invocation rate, token cost)

2. **Perform contrastive analysis where applicable.**
...
3. **NOW read `improvements.md`.**
   Reconcile fresh hypotheses against existing entries:
...
4. **Present the updated `improvements.md` diff to the human.**
...
- `prune-skill` — Remove a skill that isn't earning its context budget
...
- [ ] Test: verify stale entries are correctly identified
- [ ] Commit to skills git repo
...
- [ ] Run `/improve` for real — generate hypotheses, update improvements.md
...
- [ ] Approve 1–2 proposals in improvements.md
